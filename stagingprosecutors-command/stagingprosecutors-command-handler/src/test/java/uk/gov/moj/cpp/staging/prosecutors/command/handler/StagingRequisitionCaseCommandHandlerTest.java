...
public class StagingRequisitionCaseCommandHandlerTest {
    ...
    @InjectMocks
    SubmitRequisitionCaseCommandHandler submitRequisitionCaseCommandHandler;
    @Mock
    ...
    public void shouldHandleSummonsCaseSubmission() throws IOException, EventStreamException {
        ...
        final SubmitRequisitionProsecution submitRequisitionProsecution =
                handlerTestHelper.convertFromFile("json/submitRequisitionProsecution.json", SubmitRequisitionProsecution.class);

        final Envelope<SubmitRequisitionProsecution> envelope =
                envelopeFrom(metadataFor("stagingprosecutors.command.requisition-prosecution", SUBMISSION_ID), submitRequisitionProsecution);
        submitRequisitionCaseCommandHandler.handleRequisitionProsecutionSubmission(envelope);

        matchEvent(verifyAppendAndGetArgumentFrom(eventStream),
                "stagingprosecutors.event.prosecution-received",
                handlerTestHelper.convertFromFile("json/requisition_prosecution_received.json", JsonValue.class));
    }
