...
public class IndividualToProsecutionCaseFileIndividualConverter implements Converter<Defendant, Individual> {
    ...
    public Individual convert(final Defendant defendant) {
        ...
        return individual()
                ...
                .withCustodyStatus(individual.getCustodyStatus() != null ? individual.getCustodyStatus() : null)
                .withBailConditions(individual.getBailConditions() != null ? individual.getBailConditions() : null)
                .withParentGuardianInformation(buildParentGuardianInformation(individual.getParentGuardian() != null ? individual.getParentGuardian() : null))
                .build();
    ...
    private PersonalInformation buildPersonalInformationForIndividual(final Defendant defendant) {
        return PersonalInformation.personalInformation()
                .withAddress(buildAddress(ofNullable(defendant.getDefendantDetails()).filter(Objects::nonNull).map(DefendantDetails::getAddress).orElse(null)))
                .withContactDetails(buildIndividualContactDetails(defendant))
                .withFirstName(ofNullable(defendant.getIndividual()).filter(Objects::nonNull).map(uk.gov.moj.cpp.staging.prosecutors.json.schemas.Individual::getNameDetails).filter(Objects::nonNull).map(NameDetails::getForename).orElse(null))
                .withLastName(ofNullable(defendant.getIndividual()).filter(Objects::nonNull).map(uk.gov.moj.cpp.staging.prosecutors.json.schemas.Individual::getNameDetails).filter(Objects::nonNull).map(NameDetails::getSurname).orElse(null))
                .withGivenName2(ofNullable(defendant.getIndividual()).filter(Objects::nonNull).map(uk.gov.moj.cpp.staging.prosecutors.json.schemas.Individual::getNameDetails).filter(Objects::nonNull).map(NameDetails::getForename2).orElse(null))
                .withGivenName3(ofNullable(defendant.getIndividual()).filter(Objects::nonNull).map(uk.gov.moj.cpp.staging.prosecutors.json.schemas.Individual::getNameDetails).filter(Objects::nonNull).map(NameDetails::getForename3).orElse(null))
                .withObservedEthnicity(ofNullable(defendant.getIndividual()).filter(Objects::nonNull).map(uk.gov.moj.cpp.staging.prosecutors.json.schemas.Individual::getObservedEthnicity).filter(Objects::nonNull).map(BigDecimal::intValue).orElse(null))
                .withOccupation(ofNullable(defendant.getIndividual()).filter(Objects::nonNull).map(uk.gov.moj.cpp.staging.prosecutors.json.schemas.Individual::getOccupation).orElse(null))
                .withOccupationCode(ofNullable(defendant.getIndividual()).filter(Objects::nonNull).map(uk.gov.moj.cpp.staging.prosecutors.json.schemas.Individual::getOccupationCode).orElse(null))
                .withTitle(ofNullable(defendant.getIndividual()).filter(Objects::nonNull).map(uk.gov.moj.cpp.staging.prosecutors.json.schemas.Individual::getNameDetails).filter(Objects::nonNull).map(NameDetails::getTitle).orElse(null))
                .build();
    }
    ...
    private ParentGuardianInformation buildParentGuardianOrganisation(final ParentGuardian parentGuardian) {
        return parentGuardianInformation()
                .withOrganisationName(ofNullable(parentGuardian.getOrganisation()).filter(Objects::nonNull).map(ParentGuardianOrganisation::getOrganisationName).orElse(null))
                .withCompanyTelephoneNumber(ofNullable(parentGuardian.getOrganisation()).filter(Objects::nonNull).map(ParentGuardianOrganisation::getCompanyTelephoneNumber).orElse(null))
    ...
    private ParentGuardianInformation buildParentGuardianIndividual(final ParentGuardian parentGuardian) {
        final String observedEthnicity = ofNullable(parentGuardian.getIndividual()).filter(Objects::nonNull).map(ParentGuardianIndividual::getObservedEthnicity).filter(Objects::nonNull).map(BigDecimal::toString).orElse(null);
        final String selfDefinedEthnicity = ofNullable(parentGuardian.getIndividual()).filter(Objects::nonNull).map(ParentGuardianIndividual::getSelfDefinedEthnicity).filter(Objects::nonNull).map(String::toString).orElse(null);
        ...
        final ParentGuardianInformation.Builder parentGuardianBuilder = parentGuardianInformation()
                ...
                .withDateOfBirth(ofNullable(parentGuardian.getIndividual()).filter(Objects::nonNull).map(ParentGuardianIndividual::getDateOfBirth).orElse(null));
    ...
    private PersonalInformation buildPersonalInformation(final ParentGuardian parentGuardian) {
        ...
        return PersonalInformation.personalInformation()
                ...
                .withFirstName(ofNullable(parentGuardian.getIndividual()).filter(Objects::nonNull).map(ParentGuardianIndividual::getNameDetails).filter(Objects::nonNull).map(ParentGuardianNameDetails::getForename).orElse(null))
                .withLastName(ofNullable(parentGuardian.getIndividual()).filter(Objects::nonNull).map(ParentGuardianIndividual::getNameDetails).filter(Objects::nonNull).map(ParentGuardianNameDetails::getSurname).orElse(null))
                .withGivenName2(ofNullable(parentGuardian.getIndividual()).filter(Objects::nonNull).map(ParentGuardianIndividual::getNameDetails).filter(Objects::nonNull).map(ParentGuardianNameDetails::getForename2).orElse(null))
                .withGivenName3(ofNullable(parentGuardian.getIndividual()).filter(Objects::nonNull).map(ParentGuardianIndividual::getNameDetails).filter(Objects::nonNull).map(ParentGuardianNameDetails::getForename3).orElse(null))
                .withObservedEthnicity(ofNullable(parentGuardian.getIndividual()).filter(Objects::nonNull).map(ParentGuardianIndividual::getObservedEthnicity).filter(Objects::nonNull).map(BigDecimal::intValue).orElse(null))
                .withOccupation(null)
                .withOccupationCode(null)
                .withTitle(ofNullable(parentGuardian.getIndividual()).filter(Objects::nonNull).map(ParentGuardianIndividual::getNameDetails).filter(Objects::nonNull).map(ParentGuardianNameDetails::getTitle).orElse(null))
                .build();
    }
    ...
    private uk.gov.moj.cpp.prosecution.casefile.json.schemas.ContactDetails buildParentGuardianContactDetails(final ParentGuardian parentGuardian) {
        final Optional<ContactDetails> contactDetailsOptional = ofNullable(parentGuardian.getIndividual()).filter(Objects::nonNull).map(ParentGuardianIndividual::getContactDetails).map(Optional::ofNullable).orElse(empty());
    ...
    private uk.gov.moj.cpp.prosecution.casefile.json.schemas.ContactDetails buildIndividualContactDetails(final Defendant defendant) {
        final Optional<ContactDetails> contactDetailsOptional = ofNullable(defendant.getIndividual()).filter(Objects::nonNull).map(uk.gov.moj.cpp.staging.prosecutors.json.schemas.Individual::getContactDetails).map(Optional::ofNullable).orElse(empty());
    ...
    private uk.gov.moj.cpp.prosecution.casefile.json.schemas.SelfDefinedInformation buildSelfDefinedInformation(final Defendant defendant) {
        ...
        return selfDefinedInformation()
                .withEthnicity(defendant.getIndividual() != null ? defendant.getIndividual().getEthnicity() : null)
                .withGender(getGender(defendant))
                .withDateOfBirth(defendant.getIndividual() != null ? defendant.getIndividual().getDateOfBirth() : null)
                .build();
    }
    ...
    private Gender getGender(final Defendant defendant) {
        ...
        final uk.gov.moj.cpp.staging.prosecutors.json.schemas.Gender gender = ofNullable(defendant.getIndividual()).filter(Objects::nonNull).map(uk.gov.moj.cpp.staging.prosecutors.json.schemas.Individual::getGender).orElse(null);
